...
> **提示** 在本章的许多例子中，我们将使用操作系统的命令行界面（也称为“shell”），通过“terminal”应用程序访问。shell会显示提示; 您输入一个命令; shell响应一些文本，并为您的下一个命令显示新提示。提示可能在您的系统上看起来不同，但在以下示例中，它由符号`$`表示。在示例中，当您在符号`$`后面看到文本时，不要键入符号，而是在其后面键入命令，然后按Enter执行该命令。在示例中，每个命令下面的行是操作系统对该命令的响应。当你看到下一个前缀`$`时，应该继续输入下一个新的命令，可以一直重复这个过程。
...
当git克隆操作完成后，您将在目录*bitcoin*中拥有源代码存储库的完整本地副本。在提示符下键入“cd bitcoin”，进入为此目录：
...
默认情况下，本地副本将与最新的代码同步，这可能是不稳定的或Beta版的bitcoin。在编译代码之前，先通过检出一个发布标签（`release tag`）来选择特定版本。这将使本地副本与关键字标签所标识的代码库的特定快照同步。开发人员使用标签来按版本号标记代码的特定版本。首先，要找到可用的标签，我们使用`git tag`命令：
...
一旦您确信正在加载正确的设置并按预期运行，您可以按Ctrl-C中断进程。要在后台运行Bitcoin Core作为进程，请使用守护程序选项启动它，如`bitcoind -daemon`。要监视比特币节点的进度和运行状态，请使用命令`bitcoin-cli getinfo`：
...
交易解码展示这笔交易的所有成分，包括交易的输入及输出。在这个例子中，我们可以看到这笔给我们新地址存入50mBTC的交易使用了一个输入并且产生两个输出。这笔交易的输入是前一笔确认交易的输出（展示位以d3c7开头的 vin txid）。两个输出则是50mBTC存入额度及返回给发送者的找零。

我们可以使用相同命令（例如 `getrawtransaction` ）通过检查由本次交易的txid索引的前一笔交易进一步探索区块链。通过从一笔交易跳到另外一笔交易，我们可以追溯一连串的交易，因为币值一定是从一个拥有者的地址传送到另一个拥有者的地址。

## 3.3.2 探索区块

命令： getblock 、 getblockhash

探索区块类似于探索交易。但是，块可以由块高度或块哈希引用。首先，让我们找到一个块的高度。在买咖啡故事中，我们看到Alice的交易已被包含在块277316中。我们使用getblockhash命令，它将块高度作为参数，并返回该块的块哈希值：
...
下面的例子通过运行getinfo中的Python脚本进行简单的getinfo调用，并从Bitcoin Core返回的数据中打印区块参数。
...
在检索区块并添加所有交易输出时，我们首先检索区块277316，然后通过引用每个交易ID来检索其中的419个事务中的每一个。接下来，我们遍历每个交易的输出并将值相加。
